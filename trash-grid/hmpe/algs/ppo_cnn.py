...
class PPO:
    ...
    def __init__(self, obs_shape, action_dim, actor_lr, critic_lr,
        ...
        self.actor = ActorNet(obs_shape, action_dim, use_text, goal_space=goal_space, embedding_size=embedding_size).to(device)
        self.critic = CriticNet(obs_shape, use_text, goal_space=goal_space, embedding_size=embedding_size).to(device)
    ...
    def take_action(self, obs, goal=0):
        obs = torch.tensor([obs], dtype=torch.float).to(self.device)
        # goal = torch.tensor([goal], dtype=torch.float).to(self.device)
        with torch.no_grad():
            probs = self.actor(obs, torch.LongTensor([goal]).to(self.device))
    ...
    def update(self, transition_dict):
        ...
        goals = torch.zeros(dones.shape, dtype=torch.long).to(self.device)

        if self.use_text:
            goals = torch.LongTensor(transition_dict['goals']).to(self.device)
        ...
        td_target = rewards + self.gamma * self.critic(next_obs, goals) * (1 -
                                                                       dones)
        # print('td_target', td_target.shape)
        td_delta = td_target - self.critic(obs, goals)
        advantage = compute_advantage(self.gamma, self.lmbda,
                                               td_delta.cpu()).to(self.device)
        old_log_probs = torch.log(self.actor(obs, goals).gather(1,
                                                            actions)).detach()
        ...
        for _ in range(self.epochs):
            log_probs = torch.log(self.actor(obs, goals).gather(1, actions))
