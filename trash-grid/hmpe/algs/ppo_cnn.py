...
from model.acnet import ActorNet, CriticNet
...
class PPO:
    ...
    def __init__(self, obs_dim, share_obs_dim, hidden_dim,action_dim, actor_lr, critic_lr,
        ...
        self.actor = ActorNet(obs_dim, hidden_dim, action_dim, use_text, goal_space=goal_space, embedding_size=embedding_size).to(device)
        if use_obs:
            self.critic = CriticNet(obs_dim, hidden_dim, use_text, goal_space=goal_space, embedding_size=embedding_size).to(device)
        else:
            self.critic = CriticNet(share_obs_dim, hidden_dim, use_text, goal_space=goal_space, embedding_size=embedding_size).to(device)
    ...
    def take_action(self, obs, goal=0, avail_act = None):
        obs = torch.tensor(obs, dtype=torch.float32).to(self.device)
        with torch.no_grad():
            logits = self.actor(obs, torch.LongTensor(goal).to(self.device))
        ...
        return action_dist.probs,action,log_probs,entropy
    
    def eval_action(self, obs, goal=0, avail_act = None):
        obs = torch.tensor(obs, dtype=torch.float32).to(self.device)
        with torch.no_grad():
            logits = self.actor(obs, torch.LongTensor(goal).to(self.device))
            logits_copy = logits.clone()
        ...
        return action_dist.probs,action
    
    def get_values(self, obs, goal=0):
        obs = torch.from_numpy(obs).float().to(self.device)
        # goal = torch.tensor([goal], dtype=torch.float).to(self.device)
        with torch.no_grad():
            values = self.critic(obs, torch.tensor(goal).long().to(self.device))
        return values


    def update(self, obs, states, actions, actions_log_probs, subgoals, rewards, dones, values, returns, avail_acts= None):
        ...
        for i in range(data_chunks):
            ...
            if avail_acts is not None:
                actor_loss, critic_loss = self.train_loss(obs[idx], states[idx], actions[idx], actions_log_probs[idx], goals[idx], 
                            rewards[idx], dones[idx], values[idx], returns[idx], advantages[idx], avail_acts[idx])
            else:
                actor_loss, critic_loss = self.train_loss(obs[idx], states[idx], actions[idx], actions_log_probs[idx], goals[idx], 
                            rewards[idx], dones[idx], values[idx], returns[idx], advantages[idx])
    ...
    def train_loss(self, obs, states, actions, old_log_probs ,goals, rewards, dones, pres_values, returns, advantages, avail_acts = None):

        logits = self.actor(obs, goals.unsqueeze(1))
        ...
        if states.shape[-1] != 1:
            values = self.critic(states, goals.unsqueeze(1)).unsqueeze(-1)
        else:
