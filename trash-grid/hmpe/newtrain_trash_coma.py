...
def evaluate_policy(env, policy, logdir=None):
    ...
    for _ in range(times):
        ...
        while not done:
            for i, agent in enumerate(env.n_agents):
                ...
                action = policy.choose_action(obs[agent], deterministic=True)  # We use the deterministic policy during the evaluating
                f.write(f"[action]: {astr[action]}\n")
                s_, r, _, done, _ = env.step(agent, action)
                episode_reward += r
                obs_ = env.updateobs()
...
if __name__ == '__main__':
    ...
    env = hmpe_v2(max_cycles=max_episode_steps, is_goaltrain=False)
    env_evaluate = hmpe_v2(max_cycles=max_episode_steps, is_goaltrain=False)  # When evaluating the policy, we need to rebuild an environment
    # Set random seed
    alg = 'coma'
    seed = 0
    # env.seed(seed)
    ...
    state_dim = env.observation_space + env.goal_space
    ...
    # single agent policy net
    agent = A2C(state_dim, action_dim)
    # multi agent policy net

    t = time.strftime("%m-%d-%H%M", time.localtime())
    writer = SummaryWriter(log_dir='ppotrain/runs/{}_{}_{}_{}'.format(env_name, alg, seed, t))  # Build a tensorboard
    ...
    while total_steps < max_train_steps:
        ...
        while not done:
            episode_steps += 1
            for i, rob in enumerate(env.n_agents):
                a = agent.choose_action(obs[rob], deterministic=False)
                s_, r, _, done, info = env.step(rob, a)
                obs_ = env.updateobs()
                ...
                agent.learn(obs[rob], a, r, obs_[rob], dw)
                obs = obs_

                # Evaluate the policy every 'evaluate_freq' steps
                if (total_steps + 1) % evaluate_freq == 0:
                    evaluate_num += 1
                    evaluate_reward = evaluate_policy(env_evaluate, agent, writer.log_dir)
                    evaluate_rewards.append(evaluate_reward)
                    print("evaluate_num:{} \t evaluate_reward:{} \t".format(evaluate_num, evaluate_reward))
                    writer.add_scalar('step_rewards_{}'.format(env_name), evaluate_reward, global_step=total_steps)
                    # Save the rewards
                    # if evaluate_num % 10 == 0:
                    #     np.save('./data_train/A2C_env_{}_number_{}_seed_{}.npy'.format(env_name, number, seed), np.array(evaluate_rewards))
