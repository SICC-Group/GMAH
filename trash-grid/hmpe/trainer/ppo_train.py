...
def ppo_train_smac_policy(env, policy, num_episodes_step, logger=None, num_agents = 3, buffer_size = 32, max_cycles = 128, run_dir = None, test_episodes = 10, n_rollout_threads = 1, log_interval = 3000, eval_interval = 10000):
        ...
        with tqdm(total=buffer_size, desc='Iteration %d' % i) as pbar:
            for i_episode in range(buffer_size):
                ...
                while t < max_cycles:
                    ...
                    value = policy.get_values(np.repeat(share_obs,num_agents,axis=0), goals)
                    ...
                    states[i_episode,t] = np.repeat(share_obs,num_agents,axis=0)
                ...
                _,action,_,_ = policy.take_action(obss[0], goals, avail_action[0])
                value = policy.get_values(np.repeat(share_obs,num_agents,axis=0), goals)
                obs[i_episode,t] = obss[0]
                states[i_episode,t] = np.repeat(share_obs,num_agents,axis=0)
                avail_acts[i_episode,t] = avail_action[0]
                ...
                if (total_env_steps - last_eval_t)/eval_interval >= 1:
                    low_eval_smac_policy(env, policy, total_env_steps, num_agents, max_cycles, run_dir, test_episodes, n_rollout_threads)
                    last_eval_t = total_env_steps
            
            for _ in range(ppo_epoch):
                actor_loss, critic_loss = policy.update(obs, states, actions, actions_log_probs, subgoals, rewards, dones, values, returns, avail_acts)
...
def low_eval_smac_policy(env, policy, i_x, num_agents = 3, max_cycles = 128, run_dir = None, test_episodes = 10, n_rollout_threads = 1):
...
def ppo_train_policy(env, policy, num_episodes_step, logger=None, num_agents = 3, buffer_size = 32, max_cycles = 128, run_dir = None, test_episodes = 10, n_rollout_threads = 1, log_interval = 3000, eval_interval = 10000):
    ...
    while total_env_steps < num_episodes_step:
        ...
        with tqdm(total=buffer_size, desc='Iteration %d' % i) as pbar:
            for i_episode in range(buffer_size):
                ...
                if (total_env_steps - last_eval_t)/eval_interval >= 1:
                    low_eval_policy(env, policy, total_env_steps, num_agents, max_cycles, run_dir, test_episodes, n_rollout_threads)
                    last_eval_t = total_env_steps
...
def low_eval_policy(env, policy, i_x , num_agents = 3, max_cycles = 128, run_dir = None,test_episodes = 10, n_rollout_threads = 1):
